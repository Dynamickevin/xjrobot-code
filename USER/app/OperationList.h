...
u32   ListDelItem(ListCtrl* pCtrl,List* toDel);  //删除一个元素，成功返回0 其他方法错误标志
...
typedef struct OperationQueue_tag
{
    ...
    u16                 queueState ;    //队列状态； 处于 无，队列中，处理中，完成，无  状态循环中某个状态
...
//从操作队列中删除一个元素
static __inline u32  OperationQueueDel(ListCtrl* pCtrl,OperationQueue* toDel)
{
    u32 rt = ListDelItem(pCtrl, (List*)(toDel) ) ;
